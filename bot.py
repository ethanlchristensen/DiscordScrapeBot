...
class DiscordScrapeBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        for guild in self.guilds:
            guild_status = await loop.run_in_executor(
                self.executor,
                lambda g=guild: self.guild_status_collection.find_one(
                    {"guild_id": g.id}
                ),
            )
            ...
            await loop.run_in_executor(
                ...
                lambda g=guild: self.guild_status_collection.update_one(
                    {"guild_id": g.id},
                    {"$set": {"guild_name": g.name, "last_boot": self.boot_time}},
                    upsert=True,
                ),
            )
    ...
    async def close(self):
        ...
        for guild in self.guilds:
            await loop.run_in_executor(
                ...
                lambda g=guild: self.guild_status_collection.update_one(
                    {"guild_id": g.id},
                    {"$set": {"guild_name": g.name, "last_shutdown": shutdown_time}},
                    upsert=True,
                ),
            )
    ...
    async def catch_up_guild_messages(
        ...
        for channel in guild.text_channels:
            ...
            try:
                ...
                logger.warning(
                    f"Cannot access messages in {channel.name} of {guild.name}"
                )
            except Exception as e:
    ...
    def generate_message_payload(
        self, message: discord.Message, is_catchup: bool = False
    ) -> dict:
        ...
        try:
            ...
            await loop.run_in_executor(
                ...
                lambda: self.messages_collection.update_one(
                    {"message_id": message.id}, {"$set": payload}, upsert=True
                ),
            )
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        try:
            ...
            if result.modified_count > 0:
                logger.info(
                    f"Message {payload.message_id} marked as deleted (raw event)"
                )

        except Exception as e:
            logger.error(
                f"Error processing raw message delete {payload.message_id}: {e}"
            )
    ...
    async def on_raw_bulk_message_delete(
        ...
        try:
            ...
            logger.info(
                f"Raw bulk delete: {len(payload.message_ids)} messages marked as deleted"
            )
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.User):
        ...
        try:
            ...
            logger.info(
                f"Reaction {reaction.emoji} added to message {reaction.message.id} by {user.name}"
            )
    ...
    async def on_reaction_remove(self, reaction: discord.Reaction, user: discord.User):
        ...
        try:
            ...
            logger.info(
                f"Reaction {reaction.emoji} removed from message {reaction.message.id} by {user.name}"
            )
        ...
        for channel in target_channels:
            ...
            try:
                ...
                async for message in channel.history(
                    limit=None, after=from_datetime, before=to_datetime
                ):
                    try:
                        ...
                        logger.error(
                            f"Failed to log message {message.id} in #{channel.name}: {e}"
                        )
                        channel_failed += 1
                ...
                logger.error(
                    f"Error processing channel #{channel.name}: {e}", exc_info=True
                )
            ...
            try:
                ...
                async for message in ch.history(
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to log message {message.id} in #{ch.name}: {e}"
                        )
                        channel_failed += 1
    ...
    try:
        ...
        for cat in target_categories:
            ...
            category_info.append(
                f"**{cat.name}** ({len(text_channels_in_cat)} channels)"
            )

        if not target_channels:
            await interaction.followup.send(
                "❌ No text channels found in the specified categories!", ephemeral=True
            )
            return
        ...
        for ch in target_channels:
            ...
            try:
                ...
                logger.info(
                    f"Starting backfill for channel #{ch.name} (in {category_name})"
                )
...
async def backfill_all_guilds(
    ...
    try:
        ...
        await interaction.response.send_message(
            "❌ Invalid date format! Use: YYYY-MM-DD (e.g., 2022-01-01)", ephemeral=True
        )
    except Exception as e:
...
class BackfillConfirmView(discord.ui.View):
    ...
    def __init__(
        self, user: discord.User, from_datetime: datetime, to_datetime: datetime
    ):
    ...
    async def confirm_button(
        ...
        for guild in interaction.client.guilds:
            ...
            for channel in guild.text_channels:
                try:
                    async for message in channel.history(
                        limit=None, after=self.from_datetime, before=self.to_datetime
                    ):
                        try:
                            await interaction.client.log_message(
                                message, is_catchup=True
                            )
                            guild_success += 1
                        except Exception as e:
                            logger.error(f"Failed to log message {message.id}: {e}")
