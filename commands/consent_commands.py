...
def register_consent_commands(
    ...
    @app_commands.command(
        name="consent",
        description="Give consent for the Jade bot to log your messages"
    )
    async def give_consent(interaction: discord.Interaction):
        ...
        embed.set_footer(text="Select your consent level and click the button to proceed")
    ...
    async def check_consent_status(interaction: discord.Interaction):
        ...
        embed.set_footer(text="Use /revoke_consent to withdraw your consent at any time")
...
class ConsentModal(discord.ui.Modal, title="🤖 Jade Data Collection Consent"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Send confirmation
            level_description = self.consent_service.get_consent_level_description(self.consent_level)
            ...
            if self.backfill_historical and self.backfill_service and joined_at:
                ...
                try:
                    success, failed = await self.backfill_service.backfill_user_messages(
                    ...
                    logger.error(f"Error during retroactive backfill: {e}", exc_info=True)
                    await interaction.followup.send(
...
class RevokeConsentView(discord.ui.View):
    ...
    async def confirm_button(
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "❌ Only the command user can confirm!",
                ephemeral=True
            )
            return
    ...
    async def cancel_button(
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "❌ Only the command user can cancel!",
                ephemeral=True
            )
            return
        ...
        await interaction.response.edit_message(
            content="✅ Cancelled. Your consent remains active.",
            view=self
        )
...
class ConsentInfoView(discord.ui.View):
    ...
    async def select_consent_level(
        ...
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                "❌ Only the command user can select a level!",
                ephemeral=True
            )
            return
    ...
    async def select_backfill_option(
        ...
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                "❌ Only the command user can select this option!",
                ephemeral=True
            )
            return
